...
Of particular interest for the discussion given below is the lipid form factor as a ruler for the MD simulation quality. In the case of free-floating, non-interacting, unilamellar vesicles, the form factor can be directly obtained from the scattering experiment after a simple transformation~\cite{Kucerka08a}. For multibilayers $F(q)$ is obtained by modeling the SLD (see e.g.~\cite{Heftberger.2014}). In the first case care needs to be taking in subtracting background from water, cell and incoherent scattering. In the second case it is additionally necessary to fit the data with good accuracy. If this is ensured, experimental values for form factors compare well between different laboratories (Fig.~\ref{FormF_comp}), demonstrating the robustness of this experimental observable.
