...
Small-angle X-ray or neutron scattering (SAXS/SANS)) experiments probe the overall structure of 
the lipid bilayer and in particular membrane density profile along normal axis, membrane thickness, 
area per lipid or lipid chain length. The measured scattering intensity can be written as 
$I(q)\sim|F(q)|^2S(q) / LF$, where $F(q)$ is the bilayer form 
factor, $S(q)$ is the structure factor and $LF$ the Lorentz correction ($LF = q^2$ 
for free-floating lipid vesicles and $LF=q$ for aligned bilayers). 
...
itself along the bilayer normal. For multibilayers in the 
fluid phase, $S(q)$ is given by the Caill\'{e} theory~\cite{Zhang.1994,Lyatskaya.2001}, for 
positionally uncorrelated unilamellar vesicles $S(q)=1$~\cite{Marquardt2015}.

The form factor is given by the Fourier transform of the scattering length density (SLD),
$f_\alpha(q_z)$,  which can be written, following the notation from Ref. \cite{kucerka10} as 
...
For symmetric lipid bilayers Eq.~\ref{FF} simplifies to
\begin{equation}\label{FFsimpl}
|F(q)|=|\int_{-D/2}^{D/2}\Delta \rho_e(z) \cos(zq_z) {\rm d}z|,
\end{equation}
where $\Delta \rho_e(z)$ is the difference between solvent and sample's SLD. X-ray photons interact with 
the sample's electron cloud, while neutron scatter off nuclei in a particular manner, leading to distinct 
contrast for different parts of the membrane. X-rays, for example are most sensitive to the electron-rich 
...
Of particular interest for the discussion given below is the lipid form factor as a ruler for the MD simulation 
quality. In the case of free-floating, non-interacting, unilamellar vesicles, the form factor can be directly 
obtained from the scattering experiment after a simple transformation~\cite{Kucerka.2011}. For multibilayers 
$F(q)$ is obtained by modeling the SLD (see e.g.~\cite{Heftberger.2014}). In the first case care needs to be 
taking in subtracting background from water, cell and incoherent scattering. In the second case it is 
additionally necessary to fit the data with good accuracy. If this is ensured, experimental values for 
form factors compare well between different laboratories (Fig.~\ref{FormF_comp}), demonstrating the robustness 
of this experimental observable.

\begin{figure*}[]
	%  \centering
	%\includegraphics[width=10.6cm]{../Fig/FormF_comp.eps}
	\caption{\label{FormF_comp}
		Comparison of reported X-ray (Panel A) and neutron (Panel B) form factors for POPC bilayers at 30$^\circ$C in different geometries.
	} 
\end{figure*}


Form factors can be determined from scattering experiments of unilamellar vesicles, multilamellar vesicles or of ordered bilayers. 
Kucerka et al.~\cite{kucerka05,kucerka07} showed that form factors determined by x-ray scattering experiments of different samples 
...
from both gives the most comprehensive picture on bilayer strucuture~\cite{kucerka08a}. Therefore form factors are quite accurate and 
robust among different sample topologies and scattering techniques given that experimental care has been taken. \\
