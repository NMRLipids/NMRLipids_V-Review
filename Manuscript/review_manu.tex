...
\title{Atomistic resolution structure and dynamics of lipid bilayers in simulations and experiments  \tnotemark[mytitlenote]} 
\tnotetext[mytitlenote]{This work is dedicated to the memory of respected colleague and former supervisor, Dr. Marja Hyv{\"o}nen.}
...
%\address[mytitlenote]{This work is dedicated to the memory of Dr. Marja Hyv{\"o}nen who }
...
These parameters can be also directly calculated from the classical atomistic resolution molecular dynamics
...
gives an atomistic structural interpretation of the acquired experimental data. Significant advance of MD 
...
\begin{keyword}
phosphatidylcholine\sep NMR \sep x-ray scattering \sep neutron scattering\sep form factor\sep order parameter
...
measured from living cells, lipid extracts and model systems \cite{gally81,jacobs81,scherer87}. 

The most detailed information about lipid bilayer atomistic resolution structure and dynamics has been
achieved with various Nuclear Magnetic Resonance (NMR) and scattering techniques \cite{seelig77c,jacobs81,davis83,bloom91,nagle00,pabst10,kucerka11,marquardt15}. 
...
The third quantity is obtained from elastic X-ray or neutron scattering experiments and encodes the overall structural bilayer properties.
...
as an ultimate model giving interpretation for all these experiments simultaneously.
In addition, it would be the correct atomistic resolution representation of the system with high
probability since it reproduces large amount of independently measured experimental parameters 
...
The general conclusion is that the hydrophobic acyl chain region is well described
...
can be straightforwardly calculated from Eq. \ref{orderP}. However, the sampled $\theta$ 
distributions cannot be uniquely determined from the known order parameter. Thus the experimental
...
atomistic resolution representation and used to interpret experimental order parameters.
...
Also the signs~\cite{hong95a,hong95b,gross97} and stereospecificity of C--H segments 
...
%  \centering
  ...
  \caption{\label{allOPs}
    A)  Order parameters from simulations and experiments for phosphatidylcholine headgroup and glycerol 
    ...
    Adapted with permission from Botan et al. J. Phys. Chem. B. {\bf 2015}, 119, 15075-15088 Copyright 2015 American Chemical Society.
...
hydration level \cite{bechinger91,ulrich94,mallikarjunaiah11,dvinskikh05a} and in the presence 
...
This is straightforward for hydrocarbon segments in choline, glycerol backbone, close to the 
...
and data from simulations to help the assignment~\cite{ferreira13,leftin14}. 
...
hydration level~\cite{bechinger91,ulrich94,mallikarjunaiah11,dvinskikh05a}, presence of ions~\cite{akutsu81,altenbach84,seelig87,scherer89}, 
...
showing the measured changes as a function of ion concentrations and hydration level, respectively.
...
A similar increase is observed for different phosphatidylcholine lipids in slightly different temperatures by
...
%  \centering
  ...
  \caption{\label{opIONeffect}
    ...
    ion concentrations measured by Akutsu and Seelig with $^2$H NMR~\cite{akutsu81}. 
...
to analyze the measured changes to avoid over-interpretation of minute changes observed in experiments.
...
On the other hand, the large amount of bound positive charge may decrease the $\alpha$ carbon order parameter below zero as demonstrated by the spectra measured by Altenbach and Seelig~\cite{altenbach84} for POPC with high concentrations of CaCl$_2$,
...
\newline
  \caption{\label{qsCACLeffect}
    ...
    More recent studies show that the $\alpha$ order parameter is positive in the absence of CaCl$_2$~\cite{hong95a,hong95b,gross97}.
    Thus, the most obvious interpretation is that the $\alpha$ order parameter decreases to zero when CaCl$_2$ concentration reaches 2.0M, and 
...
R or S positions in CH$_2$ segments in order to assign order parameter to the correct hydrogen~\cite{gally81,engel81}.
...
correctly and that the isomeric positions of the hydrogen must be taken into account when calculating
...
except for the C$_2$ segment of the {\it sn-2} chain having low magnitude and significant forking in all PC lipids, in contrast to C$_2$ of the chain linked to
...
various POPC/cholesterol mixtures shows good agreement with experiments for low cholesterol concentrations, 
...
agreement, although slight overestimation of the ordering effect is observed with cholesterol concentration $\ge$ 34\%~\cite{madej15}, 
...
\newline
  \caption{\label{cholTAILmadej}
    ...
    Reprinted with permission from Madej et al. J. Phys. Chem. B {\bf 2015}, 119, 12424-12435. Copyright 2015 American Chemical Society.
  } 
\end{figure*}
...
These issues were recently discussed by Botan et al. who also compared order parameters 
...
binding~\cite{akutsu81,altenbach84} were reproduced in simulations~\cite{botan15,ionpaper}.
However, especially the effect induced by Na$^+$ ion binding is strongly overestimated
...
\newline
  \caption{\label{changesDUEna}
    ...
    and ion partition observed with CHARMM36. The results are in line with the electrometer concept
...
hydrophobic region of lipid bilayers seen in simulation videos
...
Also more detailed structural interpretation has been successful for
...
it is not clear how well the models can be used for atomistic resolution interpretation of these changes.
For example, delicate lipid-cholesterol interactions are known to induce liquid-ordered and liquid-disordered
...
and decrease with cation binding were correctly reproduced by several models, 
...
The artificial specific Na$^{+}$  binding induces effectively positively charged membrane which may easily
lead to erroneous conclusion due to dominant contribution of electrostatics for various phenomena.
...
the potential artifacts arising from simulation models must be carefully taken into account.
...
With this definition the area between the correlation function and its plateau becomes $(1-S_{\rm{CH}}^2)\tau_{e}$, 
...
field strength gives a measure of relative amount of relaxation processes with the timescales $\sim$1-20ns.
...
Usually sum of 4 or more exponential are fitted to the calculated auto--correlation function and then 
...
however some authors have also used stretched exponential functions~\cite{lindahl01,wohlert06a}.
...
Single exponential function is not enough to describe relaxation observed in simulations while 4 gives a reasonable 
fit~\cite{eldho03} which is not surprising since more than one relaxation timescales are expected to be present in bilayer
lipids~\cite{pastor88,venable93,pastor02,leftin11}. The $R_{1}^{C}$ and $R_{1}^{D}$ values are straightforward to
...
\newline
  \caption{\label{RandEFFCT}
    ...
    B) $R_1^{C}$ measured with field strength corresponding to the Larmor frequency of 125 MHz for $^{13}$C (POPC bilayer at 298K) and
...
while closer to the interfacial region motional modes corresponding lower Larmor frequencies seems to over-presented
...
with large Larmor frequencies give reasonable values for both, CHARMM~\cite{eldho03,klauda12} and Berger 
...
intuitive lipid bilayer picture compared to the traditional static pictures. While the molecular sampling rates 
seems to be underestimated closer to the interface, also the sampled structures are not exactly correct
in simulations \cite{botan15}, thus the sampling rates in simulations are mainly interesting for people 
...
The successful interpretation of relaxation time measurements with MD is significantly less laborious
...
yet on the level to be used alone for interpretation for interfacial region.
...
fits with similar or better quality would be probably possible also with other type of models.
...
The structure factor characterizes the crystalline or quasi-crystalline structure of bilayer 
...
oriented multilamellar bilayers (ORI)~\cite{Lyatskaya.2001,kucerka05a} and un-oriented multilamellar vesicles (MLVs)~\cite{Heftberger.2014}.
...
(for in-plane and out-of-plane scattering contributions) to the measured scattering intensity~\cite{Lyatskaya.2001,kucerka05a}.
...
{\bf It should be noted that the form factor's sign cannot be determined directly from scattering experiments as all information on the phase is 
lost (known as the `phase-problem' in diffraction), thus $|F(q)|$ is typically reported. However, the sign is 
practically determined by models for the bilayer structure used to extract the form factor from scattering intensity~\cite{kucerka10,marquardt15}
since only completely unphysical model would reproduce incorrent signs. Thus, the sign is not a useful quantity for assessing 
the high level accuracy of simulation data.}
...
The atomic number densities $n_\alpha(z)$ are straightforward to calculate from simulations and
...
For x-ray scattering point-wise valence electron location at the atom positions is usually assumed 
...
not change the location of form factor minima but depress the peak heights in the lobes~\cite{braun11}.
...
been nowadays often replaced with more direct comparison~\cite{nagle00} using x-ray form 
...
for simulated form factor quality has been suggested~\cite{kucerka10}. In some studies also Fourier transform coefficients are
...
\newline
  \caption{\label{FFcomp}
    Comparison of experimental~\cite{pan09,kucerka07b} and simulated (using Amber Lipid14 ~\cite{madej15}) X-ray and neutron form factors for pure DOPC bilayers and DOPC/cholesterol ($x_{chol} = 30$ mol\%) mixtures. Within experimental uncertainty, simulations agree well with neutron data. For X-ray data, simulations match well DOPC experimental data at low $q$, but are slightly off for the third lobe, indicating  some differences in the lipid bilayer fine structure. The DOPC/cholesterol mixture clearly shows disagreement between model and experiment for $q > 0.3$\AA$^{-1}$. Overall, the simulated form factor minima are shifted toward lower $q$, revealing an overestimated bilayer thickening effect of cholesterol. Reprinted with permission from Madej et al. J. Phys. Chem. B {\bf 2015}, 119, 12424-12435. Copyright 2015 American Chemical Society.
  } 
\end{figure*}
...
Atomistic resolution simulations have not been able to reproduce the special cholesterol orientation, lying flat in
...
with experiments and structural parameters indicate differences especially in the 
...
would be an ultimate tool to jointly interpret the NMR and scattering data.
...
The current MD simulation models are yet not quite capable of achieving this goal.
...
From such a simulation it is difficult to filter results arising purely from simulation artifacts.
...
Markus Miettinen and Peter Heftberger are acknowledged for useful comments.
